<template>
  <div>
    <Head title="Corretores(as)" />
    <h1 class="mb-8 text-3xl font-bold">Corretores(as)</h1>
    <div class="flex items-center justify-between mb-6">
      ...
      <Link class="btn-green" href="/organizations/create">
        <span>Novo</span>
        <span class="hidden md:inline">&nbsp;Corretor(a)</span>
      </Link>
    </div>
      ...
      <table class="w-full whitespace-nowrap">
        ...
        <tbody>
          <tr v-for="organization in organizations.data" :key="organization.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            <td class="border-t">
              <Link class="flex items-center px-6 py-4 focus:text-green-500" :href="`/organizations/${organization.id}/edit`">
                {{ organization.name }}
          ...
          <tr v-if="organizations.data.length === 0">
            <td class="px-6 py-4 border-t" colspan="4">Nada foi encontrado.</td>
          </tr>
        </tbody>
      </table>
    </div>
